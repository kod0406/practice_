...
const audio = new Audio(chrome.runtime.getURL("assets/Audio/Martini Blue.flac")); //뭐 노래는 내가 좋아하는걸로 설정
...
function StopTimer() { // 알람 양식 8
    chrome.runtime.sendMessage({
        action: "showNotification",
        type: 'basic',
        iconUrl: '/assets/Pic/34.png',
        title: '타이머 정지',
        message: '타이머를 정지했습니다.....'
    });
}

function Enable_focus() {// 알람 양식 9
    chrome.runtime.sendMessage({
        action: "showNotification",
        type: 'basic',
        iconUrl: '/assets/Pic/46.png',
        title: '집중모드 활성화',
        message: '집중모드가 켜졌습니다.'
    });
}

function Disable_focus() {// 알람 양식 10
    chrome.runtime.sendMessage({
        action: "showNotification",
        type: 'basic',
        iconUrl: '/assets/Pic/46 (1).png',
        title: '집중모드 비활성화',
        message: '집중모드가 꺼졌습니다.'
    });
}
...
async function loadChatbot() {
    try {
        // chatbot_layout.html 파일을 fetch로 불러오기
        const response = await fetch(chrome.runtime.getURL('chatbot_layout.html'));
        const chatbotHtml = await response.text();
        
        // chatbot-container에 HTML 삽입
        const container = document.getElementById('chatbot-container');
        if (container) {
            container.innerHTML = chatbotHtml;
            
            // HTML이 삽입된 후 chatbot.js 스크립트 동적 로드
